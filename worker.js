...
async function market(request, env) {
  ...
  /*
    IMPORTANT:
    We do NOT generate fake OTC data.
    Verified Quotex OTC feed is not connected.
  */
  if (mode === "OTC" || mode === "QUOTEX_OTC") {
    return json({
      ok: false,
  ...
  /*
    ...
    For 1min we additionally request:
    5min + 15min + 1h
    for MTF confirmation.
  */
  const primary = await fetchTD(
    env,
    symbol,
    interval,
    MAX_CANDLES
  ...
  const primaryAnalysis = analyzeClosed(primary.candles);
  ...
  const higherIntervals = getHigherTimeframes(interval);

  const mtf = {};

  for (const tf of higherIntervals) {
    try {
      const result = await fetchTD(
        env,
        symbol,
        tf,
        100
      ...
      const a = analyzeClosed(result.candles);
  ...
  const live = primary.candles[primary.candles.length - 1];
  ...
  return json({
    ...
    livePrice: live ? live.close : null,

    trend: primaryAnalysis.trend,
    momentum: primaryAnalysis.momentum,
    pressure: primaryAnalysis.pressure,

    rsi: round(primaryAnalysis.rsi, 2),

    ema9: round(primaryAnalysis.ema9, 6),
    ema21: round(primaryAnalysis.ema21, 6),
    ema50: round(primaryAnalysis.ema50, 6),

    macd: {
      value: round(primaryAnalysis.macd, 6),
      signal: round(primaryAnalysis.macdSignal, 6),
      histogram: round(primaryAnalysis.macdHistogram, 6)
    },

    atr: round(primaryAnalysis.atr, 6),

    support: round(primaryAnalysis.support, 6),
    resistance: round(primaryAnalysis.resistance, 6),

    pattern: primaryAnalysis.pattern,

    previousCandle: primaryAnalysis.previousCandle,
    previous2Candle: primaryAnalysis.previous2Candle,

    mtf,

    closedCandles: primaryAnalysis.closedCount,
...
async function fetchTD(env, symbol, interval, outputsize) {
...
async function testAuth(env) {
  ...
  if (
    ...
    return json({
      ...
      message: "Twelve Data API authentication works."
    });
  }
...
function analyzeClosed(allCandles) {
  ...
  if (allCandles.length < MIN_CLOSED) {
    return {
      ok: false,
  ...
  /*
    ...
    We exclude it from signal calculation.
  */
  const closed =
    allCandles.length > 1
  ...
  if (closed.length < MIN_CLOSED) {
    return {
      ok: false,
      error: "NOT_ENOUGH_CLOSED_CANDLES"
    };
  }

  const closes = closed.map(x => x.close);
  const highs = closed.map(x => x.high);
  const lows = closed.map(x => x.low);
  ...
  const prev2 = closed[closed.length - 3];
  ...
  const momentum =
    closes.length >= 10
      ? last.close > closes[closes.length - 10]
        ? "BULLISH"
        : last.close < closes[closes.length - 10]
          ? "BEARISH"
          : "NEUTRAL"
  ...
  const pressure =
    candlePressure(closed.slice(-8));

  const pattern =
    detectPattern(prev, last);
...
function decide(a, mtf) {
  ...
  */
  ...
  if (a.trend === "UPTREND") {
    buy += 20;
    reasonsBuy.push("Primary trend is bullish.");
  }

  if (a.trend === "DOWNTREND") {
    sell += 20;
    reasonsSell.push("Primary trend is bearish.");
  }
  ...
  */
  ...
  if (
    ...
    buy += 14;
    reasonsBuy.push("EMA 9 > EMA 21 > EMA 50.");
  }
  ...
  if (
    ...
    sell += 14;
    reasonsSell.push("EMA 9 < EMA 21 < EMA 50.");
  }
  ...
  */
  ...
  if (a.rsi >= 52 && a.rsi <= 68) {
    buy += 10;
    reasonsBuy.push("RSI supports bullish momentum.");
  }

  if (a.rsi <= 48 && a.rsi >= 32) {
    sell += 10;
    reasonsSell.push("RSI supports bearish momentum.");
  }
  ...
  */
  ...
  if (a.momentum === "BULLISH") {
    buy += 8;
    reasonsBuy.push("Price momentum is bullish.");
  }

  if (a.momentum === "BEARISH") {
    sell += 8;
    reasonsSell.push("Price momentum is bearish.");
  }
  ...
  */
  ...
  if (a.previousCandle === "BUYER") {
    buy += 12;
    reasonsBuy.push("Previous closed candle shows buyer pressure.");
  }

  if (a.previousCandle === "SELLER") {
    sell += 12;
    reasonsSell.push("Previous closed candle shows seller pressure.");
  }
  ...
  */
  ...
  if (
    ...
    buy += 8;
    reasonsBuy.push("Recent candles show buyer pressure.");
  }
  ...
  if (
    ...
    sell += 8;
    reasonsSell.push("Recent candles show seller pressure.");
  }
  ...
  */
  ...
  if (
    a.pattern === "BULLISH_REJECTION" ||
    a.pattern === "BULLISH_ENGULFING"
  ) {
    buy += 8;
    reasonsBuy.push("Bullish candle pattern detected.");
  }

  if (
    a.pattern === "BEARISH_REJECTION" ||
    a.pattern === "BEARISH_ENGULFING"
  ) {
    sell += 8;
    reasonsSell.push("Bearish candle pattern detected.");
  }
  ...
  */
  ...
  const higher = Object.values(mtf)
    .filter(x => x && x.trend);
  ...
  */
  ...
  if (a.pressure === "STRONG SELLER") {
    buy -= 15;
  }

  if (a.pressure === "STRONG BUYER") {
    sell -= 15;
  }
  ...
  */
  ...
  if (strongBuy) {
    ...
    strength = 95;
    reasons = [
      ...reasonsSell.slice(0, 6),
      "All available higher timeframes confirm bearish direction."
    ];
  } else if (buyGate) {
    signal = "BUY";
    strength = Math.min(92, buy);
    reasons = reasonsBuy.slice(0, 7);
  } else if (sellGate) {
    signal = "SELL";
    strength = Math.min(92, sell);
    reasons = reasonsSell.slice(0, 7);
  } else {
    signal = "WAIT";
    strength = Math.min(
      74,
...
function ema(values, period) {
  ...
  for (
    let i = Math.min(period, values.length);
    i < values.length;
    i++
  ) {
    value =
...
function isValidSymbol(symbol) {
  return /^[A-Z0-9._:/-]{2,40}$/.test(symbol);
}
